...
class ZLSwipeableViewController: UIViewController {
    ...
    func nextCardView() -> UIView? {
        ...
        let rect = CGRect.init(x: cardView.frame.width / 2 - 75, y: cardView.frame.height / 2 - 75, width: 150, height: 150)
        let wordLabel = UILabel.init(frame: rect)
        if (wordsAsset.isEmpty == false && wordsAsset.count > wordIndex) {
            wordLabel.text = wordsAsset[wordIndex].word
            wordIndex += 1
        }

        wordLabel.textAlignment = .center
        wordLabel.backgroundColor = UIColor.white
        wordLabel.layer.cornerRadius = 75
