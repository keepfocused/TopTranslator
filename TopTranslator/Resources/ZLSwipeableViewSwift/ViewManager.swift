...
class ViewManager : NSObject {
    ...
    @objc func handleTap(_ recognizer: UITapGestureRecognizer) {
        ...
        UIView.transition(with: swipeableView, duration: 1.0, options: transitionOptions, animations: {
            if self.showingBack == true {
                let image = UIImage.init(named:"anime")
                //self.letterNameLabel.text = self.regularWord
                self.showingBack = false
            } else {
                let image = UIImage.init(named:"card")
